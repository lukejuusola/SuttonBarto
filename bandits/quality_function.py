...
class QualityConstant(QualityFunction): 
    ...
    def update_parameters(self, arm, r):
        return
...
class QualitySoftmaxGrad(QualityFunction):
    def __init__( self, k: int, alpha: float, estimate: Optional[QualityFunction] = None,
                  prior: Optional[np.array] = None ):
        super().__init__(k, prior)
        self.alpha = alpha
        assert self.alpha > 0
        self.pref = self.prior.copy()
        self.estimate = estimate
        if estimate is None:
            self.estimate = QualityConstant(k)
    ...
    def update_parameters( self, arm: int, r: float ) -> None:
        pol = - softmax( self.get_parameters() )
        pol[arm] += 1
        error = r - self.estimate.get_parameters()
        self.pref += self.alpha * error * pol
